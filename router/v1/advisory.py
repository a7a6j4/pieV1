...
async def getFinancialIndependence(db: db, user: Annotated[model.User, Depends(getUserRiskProfile)]):
  ...
  required_investment = annual_income / (0.07 if user.riskProfile.primary_income_currency == schemas.Currency.USD else 0.20) # required asset value to meet income
  
  gap = (required_investment - net_worth) if required_investment <= net_worth else 0
  independence = min(1, net_worth / required_investment)
  return {
